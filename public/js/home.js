  ...
  $('.zip-form').on('submit', function (e) {
    ...
    validateZip($(this), (isValid) => {
      if (!isValid) {
        ...
        if (window.location.href.indexOf('/news/') != -1) { window.location.href = '/steps?zip=' + $(this).find("input[name='zip']").val() } else { window.location.href = 'steps?zip=' + $(this).find("input[name='zip']").val() }
      }
    })
  })
